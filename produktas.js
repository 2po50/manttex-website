...
// Surasti produktą pagal ID iš URL (?id=klasikiniai)
function gautiProduktaIsURL() {
  var id = window.location.hash.replace('#', '');
  return TURINYS.produktu_detales.find(function(p) { return p.id === id; });
}
...
function uzpildytiPuslapį() {
  ...
  var sel = document.getElementById('produktas-sel');
  if (sel) {
  ...
  // Pagrindinis produkto turinys
  var produktas = gautiProduktaIsURL();
  if (!produktas) {
    document.getElementById('produktas-detail').innerHTML = '<p>Produktas nerastas. <a href="index.html">Grįžti į pagrindinį</a></p>';
    return;
  }

  document.title = 'ManttеX – ' + produktas.pavadinimas;
  var s = SPALVOS[produktas.spalva] || SPALVOS.blue;

  var privalumaiHTML = produktas.privalumai.map(function(pr) {
    return '<li>' + pr + '</li>';
  }).join('');

  document.getElementById('produktas-detail').innerHTML =
    '<div class="product-detail-text">' +
      '<span class="product-tag" style="background:' + s.tag + ';color:' + s.tagText + '">' + produktas.etikete + '</span>' +
      '<h2>' + produktas.pavadinimas + '</h2>' +
      '<p>' + produktas.aprasymas + '</p>' +
      '<ul class="feature-list">' + privalumaiHTML + '</ul>' +
      '<a href="#uzklausos" class="btn btn-primary btn-lg">Gauti pasiūlymą</a>' +
    '</div>' +
    '<div class="product-detail-img placeholder-img" style="background:' + s.bg + ';flex:0 0 460px;max-width:460px;height:340px">' +
      '<span>' + produktas.pavadinimas + ' – nuotrauka</span>' +
    '</div>';

  // Kiti produktai (be dabartinio)
  var kiti = document.getElementById('kiti-produktai');
  if (kiti) {
    var kitosKortelės = T.produktai.filter(function(p) { return p.id !== produktas.id; });
    kiti.innerHTML = kitosKortelės.map(function(p) {
      return '<a href="produktas.html#' + p.id + '" class="product-card">' +
